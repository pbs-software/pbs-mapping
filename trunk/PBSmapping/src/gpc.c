...
Modifications:
  - 12 May 2004 [Nicholas Boers]
    - all modifications enclosed in #ifdef PBSMAPPING ... #endif
    - added global variable `gpc_no_mem` which is set to TRUE if a GPC
      function FAILS
    - added setting this variable to false at the end of each globally-
      accessible function
    - modified MALLOC to reflect this new variable; made it so MALLOC
      doesn't `exit`, but `raise(SIGTERM)` instead
  - 16 May 2004 [Nicholas Boers]
    - changed #ifdef's to #ifndef's, and PBSMAPPING to STANDALONE
  - 16 Jun 2004 [Nicholas Boers]
    - changed include dir for globals.h
  - 02 Aug 2004 [Nicholas Boers]
    - updated to fix the following warnings:
      line 1099:  warning: ISO C90 does not support the `%lf' printf format
      line ~1168: warning: `contributing', `bl', `br', `tl', `tr', `yt', `dy'
                  might be used uninitialized in this function
      line ~1823: warning: `cf', `cft', `contributing', `bl', `br', `tl',
                  `tr', `yt', `dy' might be used uninitialized in this 
                  function
===========================================================================
*/
...
/* Modified: 16 Jun 2004:  NMB:
   Now includes globals.h, which include TRUE/FALSE that were previously
...
#include <float.h>
#include <math.h>
/* Modified: 16 May 2004:  NMB:
   Added additional include file when !STANDALONE. */
#ifndef STANDALONE
#include <setjmp.h>
#endif
/*
...
/* Modified: ?? May 2004:  NMB:
   Extra variables for !STANDALONE. */
#ifndef STANDALONE
short gpc_success = TRUE; /* when a user-accessible function terminates,
                             set to true; when malloc fails, set to false */
extern jmp_buf returnLocation;
#endif
...
void gpc_free_polygon(gpc_polygon *p)
{
  ...
  p->num_contours= 0;
/* Modified: ?? May 2004:  NMB:
   Return success to caller via `gpc_success' when !STANDALONE. */
#ifndef STANDALONE
  gpc_success = TRUE;
#endif
}
...
/* if !STANDALONE, the follow code will (or at least should) never 
   be called */
void gpc_write_polygon(FILE *fp, int write_hole_flags, gpc_polygon *p)
{
  ...
  for (c= 0; c < p->num_contours; c++)
  {
    ...
    /* 2 Aug 2004: NMB: changed %lf to %f; x and y are doubles */
    for (v= 0; v < p->contour[c].num_vertices; v++)
      fprintf(fp, "% .*f % .*f\n",
...
void gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
                      gpc_polygon *result)
{
  /* Modified: 02 Aug 2004:  NMB:
     Initialized some variables to prevent warning messages. */
  sb_tree       *sbtree= NULL;
