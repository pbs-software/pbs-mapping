...
/*-----------------------------------------------------------------------------
  ...
  This function's result type (Paths) is suitable for further calls to Clipper.
  ---------------------------------------------------------------------------*/
static void
join (const Paths &subj, enum ClipType op, const Paths &clip, Paths &result)
{
	 Clipper c;
...
/*-----------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
static void
join (const Paths &subj, enum ClipType op, const Paths &clip, PolyTree &result)
{
	Clipper c;
...
getNextPath (struct PolySetState &pset, ulong64 scaleFactor, Sint &pid)
{
	...
	/* removes self-intersections from p that are created by our approach to
      clipping, which maintains the original PID and does not introduce new SIDs */
	SimplifyPolygons(p);
...
/*-----------------------------------------------------------------------------
  ...
  RH: Disabled 'protectCount' because CRAN's rchk cannot determine whether
      stack imbalances occur.
  ---------------------------------------------------------------------------*/
/* static SEXP PolySetToR (PolySet &pset, int &protectCount) */
static SEXP PolySetToR (PolySet &pset)
{
	/* final result (eventually a data frame) */
	/* SEXP res = R_NilValue; */
	/* renamed this variable because 'res' is already protected by calling function */
	SEXP rrr = R_NilValue;
	...
	rrr  = PROTECT(NEW_LIST(5));        /* protect 1 (stack position) */
	rPID = PROTECT(NEW_INTEGER(count)); /* protect 2 */
	rSID = PROTECT(NEW_INTEGER(count)); /* protect 3 */
	rPOS = PROTECT(NEW_INTEGER(count)); /* protect 4 */
	rX   = PROTECT(NEW_NUMERIC(count)); /* protect 5 */
	rY   = PROTECT(NEW_NUMERIC(count)); /* protect 6 */
	/* protectCount += 6; */

	/* add the vectors to the data frame */
	SET_ELEMENT(rrr, 0, rPID);
	SET_ELEMENT(rrr, 1, rSID);
	SET_ELEMENT(rrr, 2, rPOS);
	SET_ELEMENT(rrr, 3, rX);
	SET_ELEMENT(rrr, 4, rY);

	/* obtain the attributes */
	attrClass    = PROTECT(NEW_STRING(1));      /* protect 7 */
	attrNames    = PROTECT(NEW_STRING(5));      /* protect 8 */
	attrRowNames = PROTECT(NEW_INTEGER(count)); /* protect 9 */
	/* protectCount += 3; */
...
{
/* for recording the number of calls to PROTECT so that
     upon termination we can UNPROTECT all of them.
   RH: Disabled 'protectCount' because CRAN's rchk cannot determine whether
     stack imbalances occur.
*/
	/* int protectCount = 0; */
	...
	/* for building our result; it's easier to build it in this vector and
      convert it to an R data type than build in an R type from the start */
	PolySet resultset;
	...
	/* convert the incoming vectors to the correct types;
	   need protection because input variables are re-assigned */
	sPID = PROTECT(AS_INTEGER(sPID)); /* protect 1 (stack position) */
	sSID = PROTECT(AS_INTEGER(sSID)); /* protect 2 */
	sX   = PROTECT(AS_NUMERIC(sX));   /* protect 3 */
	sY   = PROTECT(AS_NUMERIC(sY));   /* protect 4 */

	cPID = PROTECT(AS_INTEGER(cPID)); /* protect 5 */
	cSID = PROTECT(AS_INTEGER(cSID)); /* protect 6 */
	cX   = PROTECT(AS_NUMERIC(cX));   /* protect 7 */
	cY   = PROTECT(AS_NUMERIC(cY));   /* protect 8 */
	...
	if (nSubject > 1 && nClip == 0) {
		...
		/*     concatenates A op B, A op C; maintains PIDs of clip */
	...
	if (resultset.PID.size() > 0) {
		...
		/* avoid passing stack counts to other functions */
		res = PROTECT(PolySetToR(resultset)); /* protect 9 */
		/* protectCount += 1; */
		UNPROTECT(1); /* unprotect 9 */
	}
